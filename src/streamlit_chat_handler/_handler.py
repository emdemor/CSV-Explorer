...
class ChatInteraction(BaseModel):
    """
    A model representing a complete chat interaction between a user and an assistant.

    Attributes:
        id (int | str): A unique identifier for the chat interaction.
        user_prompt (str, optional): The prompt or message provided by the user.
        user_rating (int, optional): The rating given by the user for the interaction.
        user_comment (str, optional): A comment provided by the user for the interaction.
        assistant_completion (str, optional): The response or completion generated by the assistant.
        user_elements (list[StreamlitChatElement], optional): A list of UI elements associated with the user's input.
        assistant_elements (list[StreamlitChatElement], optional): A list of UI elements associated with the assistant's response.
    """
    id: int | str
    ...
    def _get_chat_element(
        ...
        **kwargs,
        ...
        if args_were_passed:
            ...
            raise ValueError(
                f"Missing required arguments for StreamlitChatElement. {var}"
            )
    ...
    with comments_popover.form(f"feedback_form_{last_index}"):
        ...
        if submitted:
            ...
            if interaction_number in st.session_state.interactions:
                ...
                setattr(
                    st.session_state.interactions[interaction_number],
                    "comment",
                    comment,
                )

            logger.info(
                f"Avaliação da interação {interaction_number} foi alterada para {rating}"
            )
            logger.info(
                f"Comentário da interação {interaction_number} foi alterado para {comment}"
            )
            persist_logs()
            st.rerun()
